      ...
      Product.find({ closedDate: { $lte: new Date() }, status: "open" })
        ...
        .then(rows => {
          ...
          rows.forEach((row, i) => {
            ...
            if (row.bid.bids.length > 0) {
              row.bid.winnerId =
                row.bid.bids[row.bid.bids.length - 1].bidderId._id;
              bidMutations.push(
                Bid.findByIdAndUpdate(
                  row.bid._id,
                  { winnerId: row.bid.winnerId },
                  { new: true }
                )
              );
              ...
              userMutations.push(
                User.findByIdAndUpdate(row.userId._id, {
                  $inc: { balance: row.bid.bids[row.bid.bids.length - 1].price }
                })
              );
              ...
              for (let i = row.bid.bids.length - 2; i >= 0; i--) {
                ...
                if (row.bid.bids[i].bidderId != null) {
                  if (
                    !returned.includes(row.bid.bids[i].bidderId._id.toString())
                  ) {
                    userMutations.push(
                      User.findByIdAndUpdate(
                        row.bid.bids[i].bidderId._id,
                        { $inc: { balance: row.bid.bids[i].price } },
                        { new: true }
                      )
                    );
                    console.log(
                      "returned to " +
                        row.bid.bids[i].bidderId.name +
                        " with amount : " +
                        row.bid.bids[i].price
                    );
                    updatedUsers.push({
          ...
          if (productMutations.length > 0)
            return Promise.all([
              Promise.all(productMutations),
              Promise.all(userMutations),
              Promise.all(bidMutations)
            ]);
        ...
        .then(arrayOfSavedProducts => {
          ...
          updatedProducts.forEach(product => {
            db.collection("biding")
              .doc(product.bid._id.toString())
              .update({
                closed: true
              })
              .then(function() {
...
Your product "${product.title}" has been sold to ${
                product.bid.winnerId.name
              } with highest bid of Rp. ${
                product.bid.bids[product.bid.bids.length - 1].price
              },- and has been added to your account balance, please check your app for more details.
...
We would like to inform you that you've won your bid for product "${
                product.title
              }" with highest bid of Rp. ${
                product.bid.bids[product.bid.bids.length - 1].price
              },-. We've included file attachment of your product in this email. Thank you very much for your purchase, please check your app for more details.
                ...
                attachments: [
                  {
                    ...
                    filename:
                      product.title +
                      "." +
                      extensionImage[extensionImage.length - 1],
...
<h2><b>Hi ${product.userId.name},</b></h2>
We're sorry to inform that your product "${
                product.title
              }" has been due with no bidders, please check your app for more details.
...
We would like to inform you that you've lost your bid for product "${
              user.product.title
            }", Rp. ${
              user.refunded
            },- of your bid has been fully refunded to your account balance, please check your app for more details.     
